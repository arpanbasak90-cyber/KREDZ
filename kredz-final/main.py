...
async def api_endorse(token: str, decision: str = Form(...)):
    ...
    """)
# ── POST /api/seal/{token} — Mentor Seal (NEW) ─────────────────────────
@app.post("/api/seal/{token}")
async def seal_credential(token: str, mentor_email: str = Form(...)):
    results = db_select("credentials", {"endorsement_token": token})
    if not results:
        raise HTTPException(status_code=404, detail="Invalid or expired token.")
    cred = results[0]

    if cred["endorsement_status"] != "approved":
        raise HTTPException(status_code=400, detail="Credential must be approved before sealing.")

    if cred.get("seal_hash"):
        raise HTTPException(status_code=400, detail="Credential is already sealed.")

    sealed_at  = datetime.now(timezone.utc).isoformat()
    seal_input = f"{cred['credential_hash']}|{mentor_email}|{sealed_at}"
    ...
    return {"seal_hash": seal_hash, "sealed_by": mentor_email, "sealed_at": sealed_at}
