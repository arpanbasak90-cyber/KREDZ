...
Output schema (return exactly this):
{
  ...
  "checkpoints": [
    {
      "name": "short label e.g. 'GitHub activity match'",
      "status": "auto_cleared" | "needs_human",
      "reason": "one sentence — why it was auto-cleared, or exactly why a human must look"
    }
  ],
...
CHECKPOINT RULES:
- Always include these checkpoints: "Date validity", "GitHub repo authenticity",
  "Description-type consistency", "Certificate/institution match".
- Mark "auto_cleared" ONLY for things you can verify with certainty from provided data
  (e.g. dates, keyword presence, repo existing).
- Mark "needs_human" for anything requiring judgment (e.g. is this genuinely original
  work, does the project scope match the claimed effort, subjective quality) —
  and give the mentor a concrete reason to look at, not a vague one."""
